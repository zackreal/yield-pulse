...
import { getExpiryRiskData } from '@/app/actions/dashboard';
...
export function ExpiryRiskHeatmap() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const heatmapData = await getExpiryRiskData();
        ...
        <div className="overflow-x-auto flex-grow">
          <table className="w-full text-left border-collapse">
            ...
            <tbody>
              ...
              {data.length === 0 && (
                <tr>
                  <td colSpan={4} className="py-6 text-center text-sm text-slate-500">Belum ada data risiko kedaluwarsa.</td>
                </tr>
              )}
            </tbody>
          </table>
